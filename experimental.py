...
#n = 50 # Порядок КФНСТ
s = 1  # Параметр s
mp.dps = 250  # точность вычислений
points_amount = 1000 # точек на контуре
cont_tol=1e-70 # точность контура
newton_tol = 1e-200 # точность метода ньютона
def m(n,countour):
    ...
    def save_to_file(data):
        k = len(data)
        directory = 'results'
        filename = "e" + str(n) + "n" + str(k) + "nodes" + ".pkl"
    ...
    print("Starting with", points_amount, "points, n = ", n)
    start_time = time.time()
    ans = get_nodes(countour,print_roots=False)
    print(f"Elapsed time: {time.time() - start_time:.4f} seconds")
    print(n,"n",len(ans))
    if len(ans)==n: save_to_file(ans)

if __name__ == "__main__":
    count = get_contour_exact(cont_tol, desired=points_amount)
    for q in range(75,101,5):
        m(q,count)